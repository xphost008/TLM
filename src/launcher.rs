...
pub fn start_launch(option: crate::rust_lib::launcher_mod::LaunchOption){
    let launch = crate::rust_lib::launcher_mod::launch_game(option.clone(), move |command| {
        let binding = crate::main_method::APP_DATA.get().expect("Cannot get AppData Value").clone();
    ...
    if let Err(e) = launch {
        match e {
            ...
            -15 => {
                println!("{}", ansi_term::Color::Red.paint("Authlib-Injector文件未找到，暂时无法使用第三方登录，请重新下载一遍。"));
            }
            -16 => {
                println!("{}", ansi_term::Color::Red.paint("无法在版本文件夹中读取到版本JSON。请重新下载一遍MC"));
            }
            -17 => {
                println!("{}", ansi_term::Color::Red.paint("已从版本json中找到inheritsFrom键，但是无法找到依赖版本。请重新下载一遍MC"));
            }
            -18 => {
                println!("{}", ansi_term::Color::Red.paint("无法解压natives文件，请重试！"));
            }
            -19 => {
                println!("{}", ansi_term::Color::Red.paint("已获取到inheritsFrom键下的版本json，但是结构不对。你是否修改了JSON的原始结构？"));
            }
            -20 => {
                println!("{}", ansi_term::Color::Red.paint("原始JSON的结构不对。"));
            }
            _ => {
